...
### Fixed
- Apple Pay payments no longer fail silently after successful authorization. `PKPaymentAuthorizationViewControllerDelegate.paymentAuthorizationViewControllerDidFinish(_:)` fires on every sheet dismissal, including the one that follows a successful `didAuthorizePayment`. The SDK was treating that callback as a user cancellation regardless of context, flipping `isPaymentInProgress = false` and tripping the `guard isRunning` check inside `PayrailsAPI.makePayment` mid-flight. `ApplePayHandler` now latches a `didAuthorize` flag inside `didAuthorizePayment` and only emits `.canceled` on dismissal when authorization did not happen. (ONB-766)
- App no longer crashes with a "continuation resumed more than once" fatal error after Apple Pay payments. The `.canceled` branch in `Session.paymentHandlerDidFinish` now nils `onResult` and `paymentHandler` after invoking the callback, matching every other terminal exit path. Belt-and-braces with the `claimTerminal()` guard introduced on the same release; either is sufficient to prevent the double-resume, but symmetry across exit paths removes the foot-gun for future contributors. (ONB-767)
- `ApplePayHandler.didAuthorizePayment` no longer leaves the merchant's continuation hanging when `payment.token.paymentData` fails to parse as JSON. Previously the branch called `paymentCompletion(.failure)` and returned without notifying the delegate — the only reason the continuation ever woke up was the bogus `.canceled` emitted by the dismissal callback (now correctly suppressed by the ONB-766 fix). The branch now emits `.error(nil)` via the delegate, matching the sibling `payloadData` guard. (ONB-766)
- Apple Pay sheet no longer stays stuck showing "Payment Failed" after a token-parse failure on iOS simulators that don't honour the system auto-dismiss-after-`paymentCompletion(.failure)` contract. Both failure branches in `didAuthorizePayment` now call `controller.dismiss(animated: true)` explicitly, matching the success branch. (ONB-766)
